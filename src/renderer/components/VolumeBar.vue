...
const { volume } = storeToRefs(usePlayStore())//0.5
const status = ref(1)
...
const updateUI = (value) => {
    status.value = value > 0.5 ? 2 : (value > 0 ? 1 : 0)//喇叭图片 0.5 => 1
    sliderRef.value.updateProgress(value)//改变ui 
}
//用于父组件初始化 0.5
const setVolume = (value) => {
    updateUI(value) //改变ui
    updateVolume(value)//改变时间声音
}
...
<template>
    <div class="volume-bar">
        <div class="volume-status" @click="toggleMute">
            ...
            <SliderBar class="volume-value" ref="sliderRef" :value="volume" :onSeek="updateVolume"></SliderBar>
        </div>
    </div>
</template>
<style>
